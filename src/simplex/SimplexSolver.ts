...
export class SimplexSolver {
    ...
    for (let i = 0; i < rows; i++) {
      ...
      for (let j = 0; j < cols; j++) {
        newTable[i][j] = this.table[i][j].sub(
          this.table[i][pivotCol].mul(newTable[pivotRow][j]),
        );
      }
    }
    this.stepNumber++;
  ...
  private prepareExpressions(matrix: Matrix, basis: number[]): Expression[] {
    ...
    for (let b = 0; b < basis.length; b++) {
      ...
      let expression: Fraction[] = new Array(cols + 1)
        .fill(null)
        .map(() => new Fraction(0)); // +1 для константы
  ...
  private substituteFn(
    ...
    for (let i = 0; i < rows; i++) {
      ...
      substitutedFn[constantCol] = substitutedFn[constantCol].add(
        matrix[i][constantCol].mul(coeff),
      );
    }
  ...
  toStringVert(): string {
    ...
    for (let i = 0; i < this.basis.length; i++) {
      ...
      for (let j = 0; j < rows; j++) {
        ...
        if (el.equals(1)) {
          const row = this.table[j].filter(
            (_, idx) => !this.basis.includes(idx),
          );
    ...
    const filteredFn = this.table[rows - 1].filter(
      (_, idx) => !this.basis.includes(idx),
    );
...
export class ExtendedSimplexSolver extends SimplexSolver {
  ...
  setBasis(newBasis: number[]): void {
    ...
    const gaussSolved = gaussWithBasis(
      this.table
        .slice(0, -1)
        .map((row) => row.slice(0, -1).concat(row[row.length - 1])),
      newBasis,
    );
